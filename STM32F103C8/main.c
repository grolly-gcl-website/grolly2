...
void AdcInit(void);
void adc121init(void);
uint16_t get_ec_value(void);
 ...
 *
 * Normal Mode
The fastest possible throughput is obtained by leaving the ADC in normal mode
at all times, so there are no power-up delays. To keep the device
in normal mode continuously, CS must be kept low until after the 10th
falling edge of SCLK after the start of a conversion (remember that
a conversion is initiated by bringing CS low). If CS is brought high after the
10th falling edge, but before the 16th falling edge, the device will remain in
normal mode, but the current conversion will be aborted, and SDATA will return
to TRI-STATE ® (truncating the output word). Sixteen
SCLK cycles are required to read all of a conversion word from the device. After sixteen
SCLK cycles have elapsed, CS may be idled either high or low until the next
conversion. If CS is idled low, it must be brought high again
before the start of the next conversion, which begins when CS is again brought
low. After sixteen SCLK cycles, SDATA returns to TRI-STATE
®. Another conversion may be started, after t QUIET has elapsed, by bringing CS low again
 *
 *
 *
 * serial interface timing diagram for the ADC is shown in Figure 3 .
 *CS is chip select, which initiates conversions on the ADC and frames
 * the serial data transfers. SCLK (serial clock) controls both the conversion
 * process and the timing of serial data. SDATA is the serial data out pin,
 * where a conversion result is found as a serial data stream.
 * Basic operation of the ADC begins with CS going low, which initiates
 * a conversion process and data transfer. Subsequent rising and falling
 * edges of SCLK will be labelled with reference to the falling edge of CS;
 * for example, "the third falling edge of SCLK" shall refer to the third
 * falling edge of SCLK after CS goes low. At the fall of CS, the SDATA pin
 * comes out of TRI-STATE ® , and the converter moves from track mode to hold
 * mode. The input signal is sampled and held for conversion on the falling
 * edge of CS. The converter moves from hold mode to track mode on the 13th
 * rising edge of SCLK (see Figure 3 ). It is at this point that the interval
 * for the T ACQ specification begins. At least 350ns must pass between the
 * 13th rising edge of SCLK and the next falling edge of CS. The SDATA pin
 * will be placed back into TRI-STATE ® after the 16th falling edge of SCLK,
 * or at the rising edge of CS, whichever occurs first. After a conversion
 * is completed, the quiet time (t QUIET ) must be satisfied before bringing
 * CS low again to begin another conversion
 * Sixteen SCLK cycles are required to read a complete sample from the ADC.
 * The sample bits (including leading zeroes) are clocked out on falling edges
 * of SCLK, and are intended to be clocked in by a receiver on subsequent rising
 * edges of SCLK. The ADC will produce three leading zero bits on SDATA, followed
 * by twelve data bits, most significant first. If CS goes low before the rising
 * edge of SCLK, an additional (fourth) zero bit may be captured by the next
...
// controls for adc121 data exchange lines
#define adc121_cs_high	(GPIOA->BSRR |= (1<<1))		// PA1
#define adc121_cs_low	(GPIOA->BRR |= (1<<1))
#define adc121_sck_high	(GPIOA->BSRR |= (1<<2))		// PA2
#define adc121_sck_low	(GPIOA->BRR |= (1<<2))
#define adc121_sdata	(uint8_t)(((GPIOA->IDR & (1<<3))>>3)&1)	// PA3

// SCK remains high between data exchange
uint16_t get_ec_value(void){
	uint16_t ec=0;
	adc121_cs_low;
	uint8_t i = 0;
	for (i=0;i<30;i++) {
		...
		adc121_sck_high;
		Delay_us_(10);
		if (i==20) {	// Go CS HIGH now to stay in Normal mode opeartion
			adc121_cs_high;
		}
	}
