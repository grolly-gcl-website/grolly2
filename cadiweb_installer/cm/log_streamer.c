...
struct sm1_struct {
	...
	// buffers
	unsigned char empty_arr[64];				// reserved space (to fulfill 100 bytes space)
			...
			switch (dstate) {
				...
				case 0:		// IDLE
					...
					if (tx_repeat>0) {
						...
						if (sm1->txbuff_ne == 1) {
						//if ((sm1->w_tx_pointer)>(sm1->r_tx_pointer) && (sm1->txbuff_ne == 1)) {
							dstate = 2;
#ifdef TEST_MODE
							...
							if (sm1->stream_status==1 && cycle_cntr%ping_div==0) {
...
void dump_csd_(unsigned int csd_packet_size, unsigned int csd_packet_offset){
	...
	csd_packet_size = (((unsigned int)RxBuffer[0])&0x00FF) + (((unsigned int)RxBuffer[1]<<8)&0xFF00);
	...
	fwrite(RxBuffer,csd_packet_size,1,write_ptr); // 
	fflush(write_ptr);
	fclose(write_ptr);
	
	printf("Written ");
	printf("%d", csd_packet_size);
	printf("bytes\n Starting from: ");
