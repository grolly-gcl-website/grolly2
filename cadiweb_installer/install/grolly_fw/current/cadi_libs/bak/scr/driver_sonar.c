...
//	  SONAR1_TIM->ARR     = 500;                   //2hz = 500ms
	  ...
	  SONAR1_TIM->CCMR1 &= ~ TIM_CCMR1_IC1PSC_0; // Program the input prescaler. In our example, we wish the capture to be performed at each valid transition, so the prescaler is disabled (write IC1PS bits to â€˜00â€™ in the TIMx_CCMR1 register).
 ...
 //     GPIOA->CRH   |= GPIO_CRH_MODE12_0;
	...
	//	  SONAR1_TIM->ARR     = 500;                   //2hz = 500ms
		  ...
		  SONAR2_TIM->CCMR1 &= ~ TIM_CCMR1_IC1PSC_0; // Program the input prescaler. In our example, we wish the capture to be performed at each valid transition, so the prescaler is disabled (write IC1PS bits to â€˜00â€™ in the TIMx_CCMR1 register).
     ...
     GPIOA->CRH      &= ~GPIO_CRH_CNF12;		// ... from PA12
     GPIOA->CRH   |= GPIO_CRH_MODE12_0;
     ...
     RCC->APB1ENR |= RCC_APB1ENR_TIM4EN;    //enable SONAR1_TIM clock
     SONAR1_TIM->PSC	= 100-1;                //set divider
     SONAR1_TIM->CR1  =  TIM_CR1_CEN;
